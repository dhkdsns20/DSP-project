...
output_folder = fullfile(desktop_path, 'filtered_data'); % 결과 저장 폴더
...
for i = 1:length(fieldNames)
    ...
    for k = 1:length(wavFiles)
        ...
        mfcc_features_ch1 = mfcc(audioData(:, 1), fs, 'NumCoeffs', 13);
        rms_energy_ch1 = sqrt(mean(audioData(:, 1) .^ 2));
        
        if size(audioData, 2) > 1 % 두 번째 채널이 있는 경우
            mfcc_features_ch2 = mfcc(audioData(:, 2), fs, 'NumCoeffs', 13);
            rms_energy_ch2 = sqrt(mean(audioData(:, 2) .^ 2));
        else
            mfcc_features_ch2 = [];
            rms_energy_ch2 = [];
        end
